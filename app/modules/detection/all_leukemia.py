"""
ALL (Acute Lymphoblastic Leukemia) Detection Module
Uses YOLOv8n for detecting Lymphoblast, Normal Lymphocyte, Smudge Cell.
Based on C-NMC / ASH datasets.
"""
...
CLASS_NAMES = ["lymphoblast", "normal_lymphocyte", "smudge_cell"]
BLAST_CLASSES = {"lymphoblast"}

CLASS_COLORS = {
    "normal_lymphocyte": (200, 200, 200),
    "lymphoblast": (0, 0, 255),
...
class ALLResult:
    ...
    total_cells: int = 0
    blast_count: int = 0
    normal_lymphocyte_count: int = 0
    smudge_count: int = 0
    blast_percentage: float = 0.0
    inference_time_sec: float = 0.0

    def compute_metrics(self) -> None:
        self.blast_count = sum(1 for d in self.detections if d.class_name == "lymphoblast")
        self.normal_lymphocyte_count = sum(1 for d in self.detections if d.class_name == "normal_lymphocyte")
        self.smudge_count = sum(1 for d in self.detections if d.class_name == "smudge_cell")
        self.total_cells = self.blast_count + self.normal_lymphocyte_count + self.smudge_count

        if self.total_cells > 0:
            self.blast_percentage = (self.blast_count / self.total_cells) * 100
        else:
            self.blast_percentage = 0.0

    def summary(self) -> Dict:
        return {
            "image": self.image_path,
            "total_detections": len(self.detections),
            "lymphoblast": self.blast_count,
            "normal_lymphocyte": self.normal_lymphocyte_count,
            "smudge_cell": self.smudge_count,
            "blast_percentage": round(self.blast_percentage, 2),
...
class ALLDetector:
    def __init__(self, weights_path: str, device: str = "cpu"):
        ...
        self.device = device
        print(f"Loaded ALL model from {weights_path} (device={device})")
    ...
    def _draw_detections(self, yolo_result, detections: List[Detection]) -> np.ndarray:
        ...
        for det in detections:
            ...
            if is_uncertain:
                ...
                thickness = 2 if det.class_name in BLAST_CLASSES else 1
...
def load_all_model(weights_path: str, device: str = "cpu") -> ALLDetector | None:
    try:
        ...
        print(f"Failed to load ALL model: {e}")
        return None